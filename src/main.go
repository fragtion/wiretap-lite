...
func (c serveCmdConfig) Run() {
		...
		Peers: []peer.PeerConfigArgs{
			{
				...
				Endpoint: func() string {
					if len(viper.GetString("Peer.endpoint")) > 0 {
						endpoint, err := net.ResolveUDPAddr("udp", (viper.GetString("Peer.endpoint")))
						check("failed to resolve endpoint DNS name for '" + viper.GetString("Peer.endpoint") + "'", err)
				...
				PresharedKey: func() string {
					if len(viper.GetString("Peer.presharedkey")) > 0 {
						return viper.GetString("Peer.presharedkey")
					} else {
						return ""
					}
				}(),
	...
	// Create stack options with necessary protocols
	opts := stack.Options{
	...
	tun, tnet, err := netstack.CreateNetTUNwithOptions(
		Addrs,
		[]netip.Addr{},
...
func main() {
	...
	rootCmd := &cobra.Command{
		Use: os.Args[0],
		Version: Version,
		Short:   "wiretap-lite " + Version,
		//Long:    `wiretap-lite is a simplified fork of wiretap.`,
		//Short: "wiretap-lite",
		//Long:  `Listen and proxy traffic into target network`,
		Run: func(cmd *cobra.Command, args []string) {
			if cmd.Flags().NFlag() == 0 && len(args) == 0 && len(os.Args) == 1 {
	...
	rootCmd.Flags().BoolVarP(&serveCmd.deleteConfig, "delete-config", "D", serveCmd.deleteConfig, "delete wireguard config file after ingesting it")
	...
	rootCmd.Flags().StringP("preshared", "", "", "wireguard preshared key of remote peer for interface")
	...
	check("error binding flag to viper", err)
	err = viper.BindPFlag("Peer.presharedkey", rootCmd.Flags().Lookup("preshared"))
	check("error binding flag to viper", err)
